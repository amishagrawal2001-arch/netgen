...
def apply_device():
    ...
    try:
        ...
        # Start background thread to configure ARP/FDB entries from EVPN routes
        # This must run AFTER VXLAN is configured and BGP EVPN has had time to exchange routes
        if vxlan_enabled and vxlan_state in ["Configured", "Partial"]:
            import threading
            def _configure_arp_fdb_from_evpn():
                try:
                    logging.info(f"[DEVICE APPLY] [ARP/FDB THREAD] Thread started for device {device_id}, waiting 10 seconds...")
                    import time
                    time.sleep(10)  # Wait for EVPN routes to be received
                    
                    logging.info(f"[DEVICE APPLY] [ARP/FDB THREAD] Configuring ARP/FDB entries from EVPN routes for device {device_id}")
                    from utils.frr_docker import FRRDockerManager
                    from utils import vxlan as vxlan_utils
                    frr_mgr = FRRDockerManager()
                    container_name = frr_mgr._get_container_name(device_id, device_name)
                    ...
                    try:
                        ...
                        logging.warning(f"[DEVICE APPLY] [ARP/FDB THREAD] Container {container_name} not found, skipping ARP/FDB configuration")
                        return
                    
                    # Get device from database to get latest VXLAN config
                    device_info = device_db.get_device(device_id)
                    if not device_info:
                        logging.warning(f"[DEVICE APPLY] [ARP/FDB THREAD] Device {device_id} not found in database")
                        return
                    ...
                    if isinstance(vxlan_cfg, dict) and "tunnels" in vxlan_cfg:
                        ...
                        logging.info(f"[DEVICE APPLY] [ARP/FDB THREAD] Processing {len(tunnels)} tunnel(s) for device {device_id}")
                        for tunnel in tunnels:
                            if isinstance(tunnel, dict) and tunnel.get("vni"):
                                try:
                                    tunnel_result = vxlan_utils.configure_vxlan_arp_fdb_from_evpn(
                                        device_id,
                                        tunnel,
                                    ...
                                    if tunnel_result:
                                        logging.info(f"[DEVICE APPLY] [ARP/FDB THREAD] ✅ ARP/FDB configured for tunnel VNI {tunnel.get('vni')}")
                                    else:
                                        logging.warning(f"[DEVICE APPLY] [ARP/FDB THREAD] Failed to configure ARP/FDB for tunnel VNI {tunnel.get('vni')}")
                                except Exception as tunnel_exc:
                                    logging.warning(f"[DEVICE APPLY] [ARP/FDB THREAD] Error configuring ARP/FDB for tunnel VNI {tunnel.get('vni')}: {tunnel_exc}")
                    else:
                        # Single tunnel format (backward compatibility)
                        logging.info(f"[DEVICE APPLY] [ARP/FDB THREAD] Processing single tunnel format for device {device_id}")
                        try:
                            single_result = vxlan_utils.configure_vxlan_arp_fdb_from_evpn(
                                device_id,
                                vxlan_cfg,
                                container_name=container_name,
                                frr_manager=frr_mgr,
                            )
                            logging.info(f"[DEVICE APPLY] [ARP/FDB THREAD] Single tunnel ARP/FDB configuration result: {single_result}")
                        except Exception as single_exc:
                            logging.warning(f"[DEVICE APPLY] [ARP/FDB THREAD] Error configuring ARP/FDB for single tunnel: {single_exc}")
                    
                    logging.info(f"[DEVICE APPLY] [ARP/FDB THREAD] ✅ ARP/FDB configuration from EVPN routes completed for device {device_id}")
                except Exception as thread_exc:
                    logging.error(f"[DEVICE APPLY] [ARP/FDB THREAD] Error in background thread: {thread_exc}", exc_info=True)
            
            arp_fdb_thread = threading.Thread(target=_configure_arp_fdb_from_evpn, daemon=True)
